...
/**
 * Cache that manages the state and processing of {@link Command}. This class is responsible for
 * storing and retrieving {@link CommandState} objects based on a given {@link Command}. It also
 * keeps track of the currently active command.
 *
...
public class CommandCache {
  ...
  /**
   * Retrieves the {@link CommandState} for the given {@link Command}. If the {@link CommandState}
   * does not exist in the cache, it is initialized with the provided {@link AbstractCommandModel}.
   *
   * @param command the {@link Command} for which the state is to be retrieved or initialized
   * @param model the {@link AbstractCommandModel} used to initialize the {@link CommandState} if it
  ...
  /**
   * Clears the cache by removing all stored command states and resetting the current command to
   * {@code null}.
   */
  public void clear() {
    currentCommand = null;
