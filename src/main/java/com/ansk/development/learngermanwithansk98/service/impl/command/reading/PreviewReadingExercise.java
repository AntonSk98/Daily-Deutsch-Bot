...
/**
 * Service that sends a preview of the {@link ReadingExercise} that is currently saved in {@link
...
public class PreviewReadingExercise extends AbstractCommandProcessor {
   ...
   */
  protected PreviewReadingExercise(
  ...
  public void applyCommandModel(AbstractCommandModel<?> model, CommandParameters parameters) {
    readingExerciseCache
        .cachedReadingExercise()
        .ifPresentOrElse(
            readingExercise -> {
              telegramClient.sendPlainMessage(
                  parameters.chatId(), "Preparing reading exercise for preview");
              telegramClient.sendReadingExercise(parameters.chatId(), readingExercise);
            },
            () ->
                telegramClient.sendPlainMessage(
                    parameters.chatId(), "No reading exercise is in cache currently"));
  }
