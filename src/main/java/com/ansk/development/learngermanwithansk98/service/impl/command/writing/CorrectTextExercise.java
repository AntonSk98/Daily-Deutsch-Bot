...
/**
 * Service responsible for processing a text and returning both the original text and its corrected
 * version.
 *
...
public class CorrectTextExercise extends AbstractPublishExerciseSupport {
  ...
  public Consumer<Long> publish() {
    return groupId -> {
      CorrectedTextCache.CorrectedTexContainer correctText =
          correctedTextCache.getCorrectedText().orElseThrow();
    ...
    return publishHookParameters -> {
      ...
      if (correctTextModel.isWithAudio()) {
        outputGateway.sendPlainMessage(
            publishHookParameters.chatId(), "Generating a voiceover for corrected text...");
        byte[] voicedCorrectedText =
            openAiClient.createSpeech(
                correctedTextCache.getCorrectedText().orElseThrow().correctedText());
        outputGateway.sendPlainMessage(
            publishHookParameters.chatId(), "The voiceover is generated... Publishing it as well!");
        outputGateway.sendCorrectedTextAudio(
            publishHookParameters.groupId(), new ByteArrayInputStream(voicedCorrectedText));
      }
    };
  }
  ...
  public void provideDynamicPrompt(
    ...
    List<String> inputParagraphs =
        Arrays.stream(correctTextModel.getTextWithCorrections().split(paragraphSeparator)).toList();

    List<String> originalParagraphs =
        inputParagraphs.stream()
            .map(CorrectionRenderer::create)
            .map(CorrectionRenderer::getSanitizedNonCorrectedText)
            .toList();

    List<String> correctedParagraphs =
        inputParagraphs.stream()
            .map(CorrectionRenderer::create)
    ...
    CorrectedTextDocumentMetadata originalParagraphsDocumentMetadata =
        new CorrectedTextDocumentMetadata(
            correctTextModel.getTopic(),
            CorrectedTextDocumentMetadata.TextType.TEXT,
            originalParagraphs);

    CorrectedTextDocumentMetadata correctedParagraphsDocumentMetadata =
        new CorrectedTextDocumentMetadata(
            correctTextModel.getTopic(),
            CorrectedTextDocumentMetadata.TextType.CORRECTION,
            correctedParagraphs);

    ExerciseDocument textWithMistakesDocument =
        correctedTextDocumentPipe.pipe(originalParagraphsDocumentMetadata);
    ExerciseDocument textWithMistakesAndCorrectionsDocument =
        correctedTextDocumentPipe.pipe(correctedParagraphsDocumentMetadata);

    outputGateway.sendPlainMessage(parameters.chatId(), "Sending the preview...");
    outputGateway.sendCorrectedText(
        parameters.chatId(), textWithMistakesDocument, textWithMistakesAndCorrectionsDocument);

    var correctedTextContainer =
        new CorrectedTextCache.CorrectedTexContainer(
            correctText, textWithMistakesDocument, textWithMistakesAndCorrectionsDocument);
