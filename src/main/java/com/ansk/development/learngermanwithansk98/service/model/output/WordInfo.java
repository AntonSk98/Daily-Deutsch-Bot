...
public record WordInfo(Long index, String word, String translation) {
  ...
  /**
   * Finds a {@code WordInfo} by its reference, which is constructed using the word's index and word
   * itself. If the reference matches the constructed reference, the word is returned.
   *
   * @param reference the reference string, expected to be in the format "index#word" or "word" (if
   *     no index)
   * @return an {@link Optional} containing this {@code WordInfo} if the reference matches,
   *     otherwise empty
   */
