...
public class CreateReadingExercise extends ReadingExerciseSupport {
   ...
   */
  protected CreateReadingExercise(
  ...
  @Override
  public ReadingExercise.TextOutput getInputText(
      AbstractCommandModel<?> model, CommandParameters parameters) {
    ReadingExerciseWithTextModel readingExerciseModel =
        model.map(ReadingExerciseWithTextModel.class);
  ...
  private ReadingExercise.TextOutput analyzeText(
      CommandParameters parameters, ReadingExerciseWithTextModel model) {
    telegramClient.sendPlainMessage(parameters.chatId(), "Analyzing the text...");
    GenericPromptTemplate analyzeText =
        new GenericPromptTemplate(promptsConfiguration.rephraseText())
            .resolveVariable(TEXT, model.getText());
    var analyzedText =
        openAiClient.sendRequest(analyzeText.getPrompt(), ReadingExercise.TextOutput.class);
    telegramClient.sendPlainMessage(parameters.chatId(), "The text is successfully analyzed.");

    if (!model.shouldDo()) {
      return new ReadingExercise.TextOutput(
          analyzedText.level(), analyzedText.title(), model.getText());
    }
