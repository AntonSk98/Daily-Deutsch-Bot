...
/**
 * Abstract base class for command models. It encapsulates common fields and methods that are
 * required by {@link AbstractCommandProcessor}.
 *
 * <p>The class supports mappings definition between input parameter keys and the associated object
 * setter that extends the abstract model.
 *
...
public abstract class AbstractCommandModel<T> {
  ...
  /**
   * Defines constants representing common property names used in command models. These properties
   * are keys that are typically associated with their respective values in command models.
   */
  ...
  @JsonIgnore private ListIterator<String> paramIterator;

  /**
   * Initializes the concrete command model. Subclasses must implement this method to set up any
   * necessary mappings or configurations specific to the command.
   *
   ...
   */
  public void setKeyValue(String awaitingKey, String input) {
    getMapping().get(awaitingKey).accept((T) this, input);
  }

  /**
   * Retrieves an iterator over the parameter keys in the command model, allowing for sequential
   * processing.
   *
  ...
  public boolean isDefined(String key) {
    Map<?, ?> map =
        new ObjectMapper().setSerializationInclusion(NON_NULL).convertValue(this, Map.class);
   ...
   */
  private LinkedHashMap<String, BiConsumer<T, String>> getMapping() {
    return mapping;
  }
}
