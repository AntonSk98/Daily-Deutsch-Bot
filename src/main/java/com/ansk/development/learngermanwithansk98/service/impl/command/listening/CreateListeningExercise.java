...
import com.ansk.development.learngermanwithansk98.config.ListeningPrompts;
...
public class CreateListeningExercise extends AbstractCommandProcessor {
  ...
  private final ListeningPrompts promptsConfiguration;
  ...
  public void applyCommandModel(AbstractCommandModel<?> model, CommandParameters parameters) {
    ...
    telegramClient.sendPlainMessage(
        parameters.chatId(), "Splitting transcribed text into paragraphs");
    GenericPromptTemplate transcribedAudioToParagraphs =
    ...
    var paragraphs =
        openAiClient.sendRequest(
            transcribedAudioToParagraphs.getPrompt(), ListeningExercise.Paragraphs.class);

    GenericPromptTemplate listeningExercisePrompt =
        new GenericPromptTemplate(promptsConfiguration.createListeningExercise())
            .resolveVariable(TEXT, transcribedAudio);

    var listeningExerciseOutput =
        openAiClient.sendRequest(
            listeningExercisePrompt.getPrompt(), ListeningExercise.Output.class);

    Objects.requireNonNull(
        listeningExerciseOutput.level(), "Listening exercise | Level cannot be null");
    Objects.requireNonNull(
        listeningExerciseOutput.title(), "Listening exercise | Title cannot be null");
    ...
    var listeningExerciseDocumentMetadata =
        new ListeningExercise.Document(
            listeningExerciseOutput.level(),
            listeningExerciseOutput.title(),
            paragraphs.paragraphs(),
            listeningExerciseOutput.tasks().stream()
                .map(ListeningExercise.Task::question)
                .toList());

    telegramClient.sendPlainMessage(
        parameters.chatId(), "Creating a document with listening exercise");
    ExerciseDocument listeningExerciseDocument =
        documentPipe.pipe(listeningExerciseDocumentMetadata);
