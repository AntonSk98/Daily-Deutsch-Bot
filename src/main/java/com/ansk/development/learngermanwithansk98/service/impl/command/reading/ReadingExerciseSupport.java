...
import com.ansk.development.learngermanwithansk98.config.ReadingPrompts;
...
public abstract class ReadingExerciseSupport extends AbstractCommandProcessor {
  ...
  private final ReadingPrompts promptsConfiguration;
   ...
   * @param promptsConfiguration See {@link ReadingPrompts}
   ...
   * @return text
   */
  public abstract ReadingExercise.TextOutput getInputText(
      AbstractCommandModel<?> model, CommandParameters parameters);
  ...
  public void applyCommandModel(AbstractCommandModel<?> model, CommandParameters parameters) {
    ...
    var readingExercise =
        new ReadingExercise(
            generatedText.title(), documentObject.level(), paragraphs, tasks, document);
   ...
   */
  public ReadingExercise.ReadingTasks generateTasks(
      CommandParameters parameters, ReadingExercise.TextOutput text) {
    telegramClient.sendPlainMessage(parameters.chatId(), "Creating reading exercise...");
    GenericPromptTemplate createExercise =
        new GenericPromptTemplate(promptsConfiguration.createReadingExercise())
            .resolveVariable(TEXT, text.text());
    var tasks =
        openAiClient.sendRequest(createExercise.getPrompt(), ReadingExercise.ReadingTasks.class);
   ...
   * @return paragraphs
   */
  public ReadingExercise.Paragraphs mapToParagraphs(
      CommandParameters parameters, ReadingExercise.TextOutput text) {
    telegramClient.sendPlainMessage(parameters.chatId(), "Splitting the text into paragraphs....");
    GenericPromptTemplate textToParagraphsPrompt =
        new GenericPromptTemplate(promptsConfiguration.textToParagraphs())
            .resolveVariable(TEXT, text.text());
    return openAiClient.sendRequest(
        textToParagraphsPrompt.getPrompt(), ReadingExercise.Paragraphs.class);
  }
   ...
   */
  public ReadingExercise.Document generateDocumentMetadata(
   ...
   */
  public ExerciseDocument pipeDocument(
      CommandParameters parameters, ReadingExercise.Document documentMetadata) {
