...
public class GenerateReadingExercise extends ReadingExerciseSupport {
   ...
   */
  protected GenerateReadingExercise(
  ...
  private ReadingExercise.TextOutput generateText(
      CommandParameters parameters, ReadingExerciseModel readingExerciseModel) {
    telegramClient.sendPlainMessage(
        parameters.chatId(),
        String.format(
            "Generating a text for level: %s. Topic of the text is '%s'",
            readingExerciseModel.getLevel(), readingExerciseModel.getTopic()));
    GenericPromptTemplate generateText =
        new GenericPromptTemplate(promptsConfiguration.generateText())
            .resolveVariable(LEVEL, readingExerciseModel.getLevel())
            .resolveVariable(TOPIC, readingExerciseModel.getTopic());

    var generatedText =
        openAiClient.sendRequest(generateText.getPrompt(), ReadingExercise.TextOutput.class);
