...
public class TelegramSenderSupport {
  ...
  /**
   * Consumer that accepts {@link ExerciseDocument} and its {@link DocumentRenderingParams} and
   * provides a consumer that sends the document.
   *
   ...
   */
  static Consumer<TelegramClient> documentSender(
      Long chatId,
      ExerciseDocument exerciseDocument,
      DocumentRenderingParams documentRenderingParams) {
    return telegramClient -> {
      try {
        if (exerciseDocument.onePage()) {
          var onePageDocument =
              onePageDocument(chatId, exerciseDocument.pages().getFirst(), documentRenderingParams);
          telegramClient.execute(onePageDocument);
          return;
        }

        var document =
            severalPagesDocuments(chatId, exerciseDocument.pages(), documentRenderingParams);
  ...
  static String questionsAndAnswersMessageBlock(Map<String, String> questionAndAnswerMap) {
    ...
    final String questionAndAnswers =
        questionAndAnswerMap.entrySet().stream()
            .map(
                questionAndAnswer ->
                    String.format(
   ...
   */
  static Consumer<TelegramClient> audioSender(
      Long chatId, InputStream audioStream, String audioName, String caption) {
    return telegramClient -> {
  ...
  private static SendMediaGroup severalPagesDocuments(
      Long chatId, List<byte[]> binaryImages, DocumentRenderingParams documentRenderingParams) {
    var builder = SendMediaGroup.builder().chatId(chatId);
    int index = 0;
    for (byte[] imageData : binaryImages) {
      InputMediaPhoto inputMedia =
          new InputMediaPhoto(new ByteArrayInputStream(imageData), "" + index);
  ...
  private static SendPhoto onePageDocument(
      Long chatId, byte[] binaryImage, DocumentRenderingParams documentRenderingParams) {
    return SendPhoto.builder()
        ...
        .parseMode(
            documentRenderingParams.imageCaption().map(DocumentCaption::parseMode).orElse(null))
        .build();
  }
  ...
  record DocumentRenderingParams(Optional<DocumentCaption> imageCaption, boolean withSpoiler) {
    static final DocumentRenderingParams NO_RENDERING_PARAMS =
        new DocumentRenderingParams(Optional.empty(), false);
  }
}
