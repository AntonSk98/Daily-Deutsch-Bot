...
import com.ansk.development.learngermanwithansk98.config.AiProperties;
...
public class OpenAiClient {
  ...
  private static final String TRANSCRIBE_AUDIO_URL =
      "https://api.openai.com/v1/audio/transcriptions";
  private static final String CREATE_SPEECH_URL = "https://api.openai.com/v1/audio/speech";

  private final RestTemplate restTemplate;
  private final AiProperties openAIConfiguration;
  private final ObjectMapper objectMapper;

  /**
   * Constructor.
   *
   * @param openAIConfiguration See {@link AiProperties}
   * @param objectMapper See {@link ObjectMapper}
   */
  public OpenAiClient(AiProperties openAIConfiguration, ObjectMapper objectMapper) {
  ...
  public <Response> Response sendRequest(String prompt, Class<Response> clazz) {
    ...
    Output output =
        restTemplate.exchange(TEXT_REQUEST_URL, HttpMethod.POST, entity, Output.class).getBody();

    String jsonResponse =
        Objects.requireNonNull(output).choices().stream()
            .findFirst()
            .map(Choice::message)
            .map(Message::content)
            .orElseThrow();
    ...
    try {
      ...
      throw new RuntimeException(
          "Failed to convert AI response to clazz: " + clazz + " Output: " + jsonResponse, e);
    }
  }
  ...
  public String transcribeAudio(InputStream audioStream) {
    ...
    HttpEntity<MultiValueMap<String, Object>> requestEntity =
        transcribeAudioRequestBody(audioStream, headers);
    ...
    return Optional.ofNullable(response.getBody())
        .map(transcription -> transcription.get("text"))
        .orElseThrow();
  }
  ...
  public byte[] createSpeech(String text) {
    ...
    ResponseEntity<byte[]> response =
        restTemplate.exchange(CREATE_SPEECH_URL, HttpMethod.POST, requestBody, byte[].class);
  ...
  private static HttpEntity<MultiValueMap<String, Object>> transcribeAudioRequestBody(
      InputStream audioStream, HttpHeaders headers) {
    try {
      ByteArrayResource byteArrayResource =
          new ByteArrayResource(audioStream.readAllBytes()) {
            @Override
      ...
      throw new RuntimeException(
          "Error occurred while creating an audio stream that is to be transcribed", e);
    }
  }

  private HttpEntity<Map<String, String>> createSpeechRequestBody(
      HttpHeaders headers, String text) {
