...
public class AudioExerciseSender {
  ...
  public void sendListeningExercise(Long chatId, ListeningExercise listeningExercise) {
    ...
    var audioDocumentCaption =
        new TelegramSenderSupport.DocumentCaption(LISTENING_EXERCISE_DOCUMENT, "HTML");
    var renderingParams =
        new TelegramSenderSupport.DocumentRenderingParams(Optional.of(audioDocumentCaption), false);
    Consumer<TelegramClient> audioExerciseSender =
        documentSender(chatId, listeningExercise.document(), renderingParams);

    String exercisePart =
        questionsAndAnswersMessageBlock(
            listeningExercise.tasks().tasks().stream()
                .collect(
                    Collectors.toMap(
    ...
    SendMessage questionsAndAnswers =
        SendMessage.builder().chatId(chatId).text(exercisePart).parseMode("HTML").build();
    try {
  ...
  public void sendPromptToEditListeningExercise(
    ...
    SendMessage text =
        SendMessage.builder()
            .chatId(chatId)
            .text(editListeningExercisePrompt.transcription())
            .build();
